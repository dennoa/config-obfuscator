...
module.exports = options => {
  ...
  let key = options.key || 'config-obfuscator-key';

  function read() {
    try {
      let decipher = crypto.createDecipher('aes192', key);
      let decoded = decipher.update(fs.readFileSync(filename, 'binary'), 'hex', 'utf8') + decipher.final('utf8');
  ...
  function write(config) {
    ...
    let encoded = cipher.update(JSON.stringify(config), 'utf8', 'hex') + cipher.final('hex'); 
